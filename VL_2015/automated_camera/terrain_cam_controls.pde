...
void draw() {
  ...
  camPos.x = terrain_w/2 + cam_pos_x; // + cam_radius*sin(cam_speed*time);
  camPos.y = -400 + cam_pos_y;
  camPos.z = terrain_w/2 + cam_pos_z; // + cam_radius*cos(cam_speed*time);
  
  
  camera (camPos.x, camPos.y, camPos.z, // position
          terrain_w/2,0,terrain_w/2*sin(frameCount*0.1),    // look at
          0,1,0);                       // up
